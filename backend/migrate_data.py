...
def seed_data():
    """
    Seeds the database with data from database.json.
    Must be called within an active Flask app context with db already initialized.
    """
    ...
    for u_data in data.get('users', []):
        ...
        for i, sub in enumerate(core_subs):
            if i < len(core_grades):
                db.session.add(AssessmentGrade(assessment_id=assessment.id, subject=sub, grade=core_grades[i], is_core=True))
        ...
        for i, sub in enumerate(elec_subs):
            if i < len(elec_grades):
                db.session.add(AssessmentGrade(assessment_id=assessment.id, subject=sub, grade=elec_grades[i], is_core=False))
